    ...
    <div class="section-body">
        <div class="row">
            <div class="col-12 col-md-6 col-lg-7">
                <div class="card">
                    <div class="card-body">
                        <form action="<?= site_url('kota'); ?>" method="POST" autocomplete="off">
                            <?= csrf_field(); ?>
                            <div class="form-group">
                                ...
                                <select name="provinsi_id" class="form-control select2" required>
                                    ...
                                    <?php foreach ($provinsi as $pro => $v_provinsi): ?>
                                        <option value="<?= esc($v_provinsi->provinsi_id); ?>"><?= esc($v_provinsi->provinsi_name); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="form-group">
                                <label><?= esc($t('city_name')) ?><small class="text-danger">*</small></label>
                                <input type="text" name="kota_name" class="form-control" required autofocus>
                            </div>
