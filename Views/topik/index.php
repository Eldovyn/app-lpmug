...
$fmt = static function (string $tpl, array $vars): string {
    ...
    <div class="section-body">
        <div class="card">
            <div class="card-header">
                <h4 class="text-sm">
                    <a href="<?= site_url('topik/new'); ?>" class="btn btn-primary">
                        <i class="fas fa-plus-circle mr-1"></i><?= esc($t('add_data')) ?>
                    </a>
                    <!-- <a href="<?= site_url('topik/trash'); ?>" class="btn btn-danger"><i class="fas fa-trash mr-1"></i>Data yang dihapus</a> -->
                </h4>
                ...
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            $no = 1 + (10 * ($page - 1));
                            foreach ($topik as $fgsi => $v_topik):
                            ?>
                                <tr>
                                    <td><?= $no++; ?></td>
                                    <td><?= esc($v_topik->topik_name); ?></td>
                                    <td class="text-center">
                                        <a href="<?= site_url('topik/' . $v_topik->topik_id . '/edit'); ?>" class="btn btn-warning btn-sm">
                                            <i class="fas fa-pencil-alt"></i>
                                        </a>

                                        <form action="<?= site_url('topik/' . $v_topik->topik_id); ?>" method="POST" class="d-inline" id="del-<?= esc($v_topik->topik_id); ?>">
                                            <?= csrf_field(); ?>
                                            <input type="hidden" name="_method" value="DELETE">
                                            <button class="btn btn-danger btn-sm"
                                                data-confirm="<?= esc($t('delete_title')) ?> | <?= esc($t('delete_msg')) ?>"
                                                data-confirm-yes="submitDel(<?= (int) $v_topik->topik_id; ?>)">
                    ...
                    <div class="float-left">
                        ...
                        $showingText = $fmt($t('showing'), ['from' => $from, 'to' => $to, 'total' => $total]);
                        ?>
                        <i><?= esc($showingText) ?></i>
                    </div>
