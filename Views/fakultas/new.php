...
$lang = $lang ?? ($request->getCookie('lang') ?? 'id'); // read cookie :contentReference[oaicite:1]{index=1}
$lang = strtolower(trim((string) $lang));
$lang = ($lang === 'en') ? 'en' : 'id'; // default id
    ...
    <div class="section-body">
        <div class="row">
            <div class="col-12 col-md-6 col-lg-7">
                <div class="card">
                    <div class="card-body">
                        <form action="<?= site_url('fakultas'); ?>" method="POST" autocomplete="off">
                            <?= csrf_field(); ?>
                            <div class="form-group">
                                <label><?= esc($t('label_name')) ?><small class="text-danger">*</small> (<?= esc($t('example')) ?>)</label>
                                <input type="text" name="fakultas_name" class="form-control" required autofocus>
                            </div>
